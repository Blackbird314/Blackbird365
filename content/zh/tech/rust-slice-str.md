...
Slice 类型非常特殊，你通不能在代码中声明 `[T]` 或 `str` 类型的变量，换言之，Slice 实例不能存放到栈上，除非使用 nightly 版本并且开启名为 `unsized_locals` 的 `feature`：
...
以 `str` 为例，它只能以 `&str` `&mut str` `Box<str>` `String` 等形式呈现，前两者是对 `str` 的引用，后两者包含了指向 `str` 的指针。
