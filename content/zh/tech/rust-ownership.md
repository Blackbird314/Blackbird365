...
> 重点辨析：既然堆内存都是由栈上指针进行管理的，那么当 `Value` 是 `String::from("xxx")` 这样的堆内存时，严格来说，`Variable` ..拥有的是栈上（宽）指针的所有权..，而非堆上字符串。只是为了方便理解，也经常会说 `Variable` 是那个堆中实际数据的所有者。
...
那么哪些类型不能实现 `Copy` 呢？任何自身或部分实现了 `Drop` Trait 的类型都不可实现 `Copy`，比如 `String` 和 `Vec<T>` 类型。除此之外，`Copy` 一个可变引用 `&mut T` 也是不安全的，这违背了引用的借用规则。
