...
为了同时兼顾安全与性能，Rust 选择了第三种方式：由编译器管理内存，即编译时就决定何时释放内存，并将相关指令在恰当位置写入可执行程序。这种方式可以理解为编译期 GC，这不会带来任何运行时开销，也可以保证内存安全和并发安全（虽然 Rust 无法完全避免内存泄漏，但至少大大降低了它发生的概率，这是后话）。
...
> 重点辨析：既然堆内存都是由栈上指针进行管理的，那么当 `Value` 是 `String::from("xxx")` 这样的堆内存时，严格来说，`Variable` ..所拥有的是栈上的（宽）指针..而非堆上字符串。只是为了方便理解，也经常会说 `Variable` 是那个堆中实际数据的所有者。
...
Rust 为了适应所有权系统，没有使用深浅拷贝的概念，而是提出了移动(move)、拷贝(copy)和克隆(clone)的区别。
...
因为声明的 `s2` 是不可变变量，你不能通过它修改字符串值，但编译期的 `mut` 标识是作用在变量名上，而不是那个内存对象。因此下面的例子中 `s1` 是不可变的，并不妨碍我们定义另外一个可变的变量名 `s3` 来写这块内存：
...
那么哪些类型不能实现 `Copy` 呢？任何自身或部分实现了 `Drop` Trait 的类型都不可实现 `Copy`，比如 `String` 和 `Vec<>` 类型。除此之外，`Copy` 一个可变引用 `&mut T` 也是不安全的，这违背了引用的借用规则。
...
为了使用方便，Rust 编译器对引用的作用域进行了优化：普通变量的作用域从创建持续到某一个花括号 `}`；引用的作用域从创建开始，一直持续到它最后一次使用的地方。这种优化行为被称为非词法作用域生命周期(Non-Lexical Lifetimes, NLL)

创建一个引用的行为称为借用(Borrowing)，即引用类型会借用（而非获得）内存对象的所有权。当你只想使用变量，而不想转移所有权时，可以通过变量的引用进行访问，例如：
...
传入 `calculate_length()` 的是 `s1` 的引用，参数 `s` 不会获得 `s1` 的所有权，如图所示：

![2024-05-24-15-42-23.png](/images/2024-05-24-15-42-23.png)

Rust 的借用规则很有趣：在同一时刻，要么只有一个可变引用(`&mut T`)，要么有任意数量的共享引用(`&T`)。可变引用与共享引用不能同时存在，也不能同时存在多个可变引用。正因如此，可变引用 `&mut T` 不能实现 `Copy`，这会违反借用规则。当然，共享引用是可 `Copy` 的，即把共享引用赋值给另一个共享引用后，可以继续使用。

为什么会有这样的规定呢？实际上 Rust 要求..在同一时刻，一份资源只能被至多一个变量名读写，或者被多个变量名读取..。由此：

- 对于不可变借用(`let y = &x`)：1)引用 `y` 是只读的；2)在 `y` 作用域结束之前，`x` 可读不可写（存在 `y` 可读），因此 `x` 能被不可变借用但不能被可变借用
- 对于可变借用(`let y = &mut x`)：1)引用 `y` 可读写；2)在 `y` 作用域结束之前，`x` 不可读不可写（存在 `y` 可写），因此 `x` 不能再次被借用
...
fn main() {
    let mut x = 0;
    let y = &mut x; // 发生了可变借用
    if x == 0 { // 存在 x 的可变引用 y，此时不能通过原变量 x 读取值，报错
        *y += 1; // y 的作用域到此结束
        println!("{}", x); // x 可以正常读取
    }
}
...
注意，Rust 中所有的值都有所有权，引用类型的值也不例外。引用不拥有指向对象的所有权，但引用变量拥有自身地址值的所有权。
...
解引用操作 `*r` 会得到一个被称为影子变量的东西，可以理解为没有所有权的变量别名。它可以用来对本体变量的内存对象进行读写操作，但无法通过影子变量进行所有权转移(Move)，这会影响本体对于内存对象的掌控：
...
fn main() {
    let num1 = 1;
    let num1_ref = &num1;
    // i32 实现了 Copy，可以针对实现了 Copy 的类型的影子变量进行副本操作，这不会影响本体的所有权
    let num2 = *num1_ref;
    let x = MyType{val: 1};
    let y = &x;
...
虽然不能通过引用(其实是自动解引用之后的影子变量)去移动所有权，但是你可以用新的内存对象替换旧的：
...
fn main() {
    ...
    let mut y = &mut x;
    y.val = String::from("bbb"); // "aaa" 自动释放
}
```
