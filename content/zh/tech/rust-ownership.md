...
Rust 的借用规则很有趣：在同一时刻，要么只有一个可变引用(`&mut T`)，要么有任意数量的共享引用(`&T`)。可变引用与共享引用不能同时存在，也不能同时存在多个可变引用。正因如此，可变引用 `&mut T` 不能实现 `Copy`，这会违反借用规则。当然，共享引用是 `Copy` 的，即把共享引用赋值给另一个共享引用后，可以继续使用。
