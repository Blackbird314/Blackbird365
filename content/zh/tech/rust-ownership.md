...
我们常用“变量”这个术语，比如let x = Box::new(1i32)，被称之为“定义值为 1 的变量 x”，这是比较简化和高度抽象的说法，实际情况更接近于“告知编译器，在此处需要在堆上分配长度为 4 字节的内存并按照 32 位 int 初始化为 1，此块源代码我将使用 x 来指代这块内存”。
...
因为声明的 `s2` 是不可变变量，你不能通过它修改字符串值，但编译期的 `mut` 标识是作用在变量名上，而不是那个内存对象。因此下面的例子中 `s1` 是只读的，并不妨碍我们定义另外一个可写的变量名 `s3` 来写这块内存：
...
编译期的约束是作用在变量名上的，而不是内存对象。比如上面的例子中 x 是只读的，并不妨碍我们定义另外一个可写的变量名来写这块内存（let mut y = x），甚至重新定义 x 来为可写的变量名（let mut x = x）。
