...
当一个值不再被需要时（比如离开作用域），Rust 会运行 "destructor" 将其释放。如果该类型实现了 `Drop` Trait，destructor 会调用 `Drop::drop` 析构函数，但即使没有实现这个 `Drop`，destructor 也会自动生成 "drop glue"，递归地为这个值的所有成员调用析构函数。因此在大多数情况下，你不必为你的类型实现 `Drop`。
