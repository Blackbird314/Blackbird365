...
为了避免重复释放资源，Rust 编译器不允许开发者手动调用 `Drop::drop` 析构函数，但偶尔又需要提前结束一个变量的生命周期(比如 `Mutex` 的守卫变量)，为此 Rust 标准库还提供了能手动调用的 `std::mem::drop` 函数，它的实现简单到令人惊异：

```Rust
pub fn drop<T>(_x: T) { }
```

这就是一个函数体为空的函数，它接受任意类型的参数，但不做任何操作。显然它利用了 Rust 的 Move 语义，原内存对象的所有权传入 `std::mem::drop` 函数后，会自动调用 "destructor"。
